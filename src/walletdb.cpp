...
CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    // First: get all CWalletTx and CAccountingEntry into a sorted-by-time std::multimap.
             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CHoneyAddress(strAddress).Get()];
        }
        else if (strType == "tx")
        {
            uint256 hash;
