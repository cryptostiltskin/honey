...
#if defined(NDEBUG)
# error "Honey cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Honey Signed Message:\n";
...
{
    std::map<uint256, CTransaction>::iterator it = mapOrphanTransactions.find(hash);
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal honey miner
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    for (CBlockIndex* pindex : vDisconnect)
    {
        ...
        for (const CTransaction& tx : reverse_iterate(block.vtx))
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    else
    {
        ...
        for (CBlockIndex *pindex : reverse_iterate(vpindexSecondary))
        {
            CBlock block;
            if (!block.ReadFromDisk(pindex))
            {
...
#ifdef ENABLE_WALLET
// honey: attempt to generate suitable proof-of-stake
