...
{
    widget->setMaxLength(HoneyAddressValidator::MaxAddressLength);
    widget->setValidator(new HoneyAddressValidator(parent));
    widget->setFont(honeyAddressFont());
}
...
bool parseHoneyURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("honey"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!HoneyUnits::parse(HoneyUnits::HONEY, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseHoneyURI(QString uri, SendCoinsRecipient *out)
{
    // Convert honey:// to honey:
    //
    //    Cannot handle this later, because honey:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("honey://"))
    {
        uri.replace(0, 12, "honey:");
    }
    QUrl uriInstance(uri);
    return parseHoneyURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open honey.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Honey.lnk";
}
...
{
    return GetAutostartDir() / "honey.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a honey.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Honey\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Honey-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  honey-qt [" + tr("command-line options") + "]                     " + "\n";
